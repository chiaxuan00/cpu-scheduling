...
public class NonPreemptiveSJF implements CpuScheduling{
    ...
    public boolean checkProcessFinished = false;
    ...
    public boolean isProcessing(int time){
        ...
        if(prevProcess==null) {
            processToBeProcessed = processWithShortestBurstTime.get(0); //run first process
            ...
            if(processToBeProcessed.hasDoneBurst()) 
                checkProcessFinished=true;
        } else{
            prevProcess.burst(time);
            checkProcessFinished = false;
         }

        if(checkProcessFinished==true){
        ...
        if(processToBeProcessed.getBurstTime() == 0){   //check burst done
    ...
    private void addProcessWithShortestBurstTime(List<Process> pList){
        ...
        for(Process p : pList){
            ...
            System.out.println("Error: No process running");
    }
